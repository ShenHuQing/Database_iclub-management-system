  ...
  methods: {
    async fetchData(clubId) {
      try {
        ...
        this.myClub = {
          id: clubId,
          name: clubDetailsResponse.data.data.name, // 社团名称
          // type: activitysResponse.data.type, // 社团活动类型
          description: clubDetailsResponse.data.data.description, // 社团描述
          // posts:activitysResponse.data.posts,
          showAddPost: (clubDetailsResponse?.data?.data?.Cid !== -1), // 判断是否显示添加帖子按钮
        };
